...
export default function Home() {
  ...
  function printRules() {
    const severityBadge = (sev: string) => {
      ...
      return `<span style="display:inline-block;padding:1px 7px;border-radius:4px;font-size:0.72rem;font-weight:600;background:${color};color:#fff;text-transform:uppercase;letter-spacing:0.04em;">${sev}</span>`;
    };

    const rows = categoryDefinitions.map((cat) => {
      const catState = config[cat.key];
      const ruleRows = cat.rules.map((ruleDef) => {
        const ruleState = catState.rules[ruleDef.key];
        const enabled = ruleState?.enabled ?? false;
        const sev = ruleState?.severity ?? "info";
        return `
          <tr style="border-bottom:1px solid #e5e7eb;${enabled ? "" : "opacity:0.4;"}">
            <td style="padding:8px 12px;font-weight:500;font-size:0.85rem;">${ruleDef.label}</td>
            <td style="padding:8px 12px;font-size:0.82rem;color:#4b5563;">${ruleDef.helper}</td>
            <td style="padding:8px 12px;white-space:nowrap;">${severityBadge(sev)}</td>
            <td style="padding:8px 12px;text-align:center;font-size:0.85rem;">${enabled ? "✓" : "—"}</td>
          </tr>`;
      }).join("");

      return `
        <div style="margin-bottom:32px;break-inside:avoid;">
          <h2 style="margin:0 0 4px;font-size:1rem;font-weight:700;color:#111;">${cat.title}</h2>
          <p style="margin:0 0 10px;font-size:0.82rem;color:#6b7280;">${cat.description}</p>
          <table style="width:100%;border-collapse:collapse;font-family:system-ui,sans-serif;">
            <thead>
              <tr style="background:#f3f4f6;text-align:left;">
                <th style="padding:7px 12px;font-size:0.75rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;width:35%;">Rule</th>
                <th style="padding:7px 12px;font-size:0.75rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;">Description</th>
                <th style="padding:7px 12px;font-size:0.75rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;width:90px;">Severity</th>
                <th style="padding:7px 12px;font-size:0.75rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;width:70px;text-align:center;">On</th>
              </tr>
            </thead>
...
<head>
  <meta charset="UTF-8" />
  <title>dbt Governance Rules — ${config.project.name || "All Rules"}</title>
  <style>
    @media print { button { display: none !important; } }
    body { font-family: system-ui, -apple-system, sans-serif; color: #111; margin: 0; padding: 32px 40px; }
  </style>
</head>
<body>
  <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:6px;">
    <h1 style="margin:0;font-size:1.4rem;font-weight:800;">dbt Governance Rules</h1>
    <button onclick="window.print()" style="padding:7px 16px;background:#111;color:#fff;border:none;border-radius:6px;font-size:0.84rem;cursor:pointer;">Print / Save PDF</button>
  </div>
  ${config.project.name ? `<p style="margin:0 0 4px;font-size:1rem;font-weight:600;color:#374151;">${config.project.name}</p>` : ""}
  ${config.project.description ? `<p style="margin:0 0 24px;font-size:0.85rem;color:#6b7280;">${config.project.description}</p>` : `<div style="margin-bottom:24px;"></div>`}
  ${rows}
  <p style="margin-top:24px;font-size:0.72rem;color:#9ca3af;">Generated by dbt-governance hub · Disabled rules are dimmed</p>
</body>
</html>`;
      ...
      <section className="workspace">
        ...
        <aside className="workspace-nav panel">
          ...
          <div className="nav-actions">
            ...
            <button className="ghost-button small" onClick={printRules} title="Open a printable view of all rules — useful to share with customers">
              Print rules
            </button>
          </div>
