"""Generate GEMINI.md project context from governance configuration."""
...
def generate_gemini_md(config: GovernanceConfig) -> str:
    """Render GEMINI.md content from a GovernanceConfig.

    Google's Gemini CLI reads GEMINI.md at the project root the same way
    Claude Code reads CLAUDE.md — it is loaded as persistent context for
    every AI-assisted review session.
    """
    lines = [
        f"# dbt Project: {config.project.name}",
        "",
        "<!-- Auto-generated from .dbt-governance.yml by dbt-governance — loaded by Gemini CLI -->",
        "",
    ]
...
def write_gemini_md(
    ...
    output_path: str | Path = "GEMINI.md",
) -> Path:
    """Write GEMINI.md to disk and return the output path."""
    resolved_config = config or load_config(config_path)
    path = Path(output_path)
    path.write_text(generate_gemini_md(resolved_config), encoding="utf-8")
    return path
